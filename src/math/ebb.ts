 ...
 */
export const xy2aa = ({ x, y }: { x: number; y: number }) => ({
  a1: x + y,
  a2: x - y,
});
 ...
 */
export const aa2xy = ({ a1, a2 }: { a1: number; a2: number }) => ({
  x: (a1 + a2) / 2,
  y: (a1 - a2) / 2,
});

// step to rate, accumulator: 2**31 rate = 1 step
export const s2rate = (hz: number) => ((2 ** 31 / 25000) * Math.abs(hz)) | 0;

// rate to step, accumulator: 2**31 rate = 1 step
export const rate2s = (rate: number) => ((rate / 2 ** 31) * 25000) | 0;

// time to intervals: 25000 intervals per second
export const t2interval = (t: number) => t * 25000;

// intervals to time: 25000 intervals per second
export const interval2t = (interval: number) => interval / 25000;
...
export const mm2steps = (mm: number, mode: number = 1) => {
  const stepsPerMm = HIGH_DPI_XY / 2 ** (mode - 1) / 25.4;
  return (stepsPerMm * mm) | 0;
};

export const steps2mm = (steps: number, mode: number) => {
  const stepsPerMm = HIGH_DPI_XY / 2 ** (mode - 1) / 25.4;
...
export const aaStepsToLMParams = (
  { a1, a2 }: { a1: number; a2: number },
  t: number,
  clear: number = 3,
) => {
...
export const servoTime = (min: number, max: number, rate: number) =>
