...
export const arcAngleFn = (ux: number, uy: number, vx: number, vy: number) => {
...
export const getMidPoint = ([x0, y0]: Point2D, [x1, y1]: Point2D): Point2D => {
  return [(x0 + x1) / 2, (y0 + y1) / 2];
};

// convert quadratic to cubic bezier
// https://codepen.io/enxaneta/post/quadratic-to-cubic-b-zier-in-svg
export function quadToCubicBezierControlPoints(
  [x0, y0]: Point2D,
  [x1, y1]: Point2D,
  [x2, y2]: Point2D,
): [Point2D, Point2D] {
